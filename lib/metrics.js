...
class MetricsEngine {
  constructor(maxHistoryDays = 30) {
    ...
    this.allTimeUserHashes = new Set(); // Persistent set of all unique hashed installs
  ...
  /**
   * Compute 30-day Monthly Active Users from today + historical buckets
   */
  computeMau() {
    const mauSet = new Set(this.today.activeUsers);
    for (const day of this.history) {
      if (Array.isArray(day.activeUserHashes)) {
        for (const h of day.activeUserHashes) {
          mauSet.add(h);
        }
      }
    }
  ...
  /**
   * Export complete metrics snapshot for Discord backup / persistent hydration
   */
  exportSnapshot() {
    return {
      version: 1,
      ...
      today: {
        ...this.today,
        activeUsers: [...this.today.activeUsers],
      },
      history: this.history,
      allTimeHashes: [...this.allTimeUserHashes],
      errorLog: this.errorLog,
    };
  }

  /**
   * Hydrate state from a previously exported snapshot
   */
  hydrateSnapshot(snapshot) {
    if (!snapshot || typeof snapshot !== "object") return false;
    try {
      if (Array.isArray(snapshot.allTimeHashes)) {
        for (const h of snapshot.allTimeHashes) {
          this.allTimeUserHashes.add(h);
        }
      }
      if (Array.isArray(snapshot.history)) {
        this.history = snapshot.history.slice(0, this.maxHistoryDays);
      }
      if (Array.isArray(snapshot.errorLog)) {
        this.errorLog = snapshot.errorLog.slice(0, 200);
      }
      if (snapshot.currentDate === this.currentDate && snapshot.today) {
        this.today.searchRequests = Math.max(this.today.searchRequests, snapshot.today.searchRequests || 0);
        this.today.proxyRequests = Math.max(this.today.proxyRequests, snapshot.today.proxyRequests || 0);
        this.today.cacheHits = Math.max(this.today.cacheHits, snapshot.today.cacheHits || 0);
        this.today.cacheMisses = Math.max(this.today.cacheMisses, snapshot.today.cacheMisses || 0);
        if (Array.isArray(snapshot.today.activeUsers)) {
          for (const u of snapshot.today.activeUsers) {
            this.today.activeUsers.add(u);
            this.allTimeUserHashes.add(u);
          }
        }
      }
      return true;
    } catch (err) {
      console.error("[METRICS] Failed to hydrate snapshot:", err);
      return false;
    }
  }
  ...
  rolloverDay(newDateStr = getTodayUtcString(), previousDateStr = this.currentDate) {
    ...
    const summary = {
      ...
      activeUserHashes: [...this.today.activeUsers],
      totalRequests,
  ...
  getLiveStats() {
    ...
    return {
      ...
      mau30d: this.computeMau(),
      allTimeInstalls: this.allTimeUserHashes.size,
      today: {
