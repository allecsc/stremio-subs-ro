...
/**
 * Exclude subtitle tracks that are incomplete or split:
 * - Forced subtitles
 * - Multi-CD / split-disc parts (CD1-9, Disc1-9, Part1-9, Pt1-9, D1-9)
 *
...
/**
 * Extract explicit season number from metadata text if explicitly identified.
 * Handles:
 * - S02E03, S2E3
 * - 2x03
 * - Season 2, Sezon 2, Saison 2, κύκλος 2, etc.
 * Returns null if ambiguous, missing, or not an explicit season match.
 *
...
function explicitSeason(text) {
  ...
  // 1. Pattern: S02E03, S2E3
  const sPattern = /\bs\s*(\d{1,2})\s*e\s*\d{1,3}\b/i;
  const sMatch = str.match(sPattern);
  if (sMatch) return parseInt(sMatch[1], 10);

  // 2. Pattern: 2x03
  const xPattern = /\b(\d{1,2})\s*x\s*\d{1,3}\b/i;
  const xMatch = str.match(xPattern);
  if (xMatch) return parseInt(xMatch[1], 10);

  // 3. Pattern: Season 2, Sezon 2, Saison 2, Staffel 2, κύκλος 2, etc.
  const wordPattern = new RegExp(`${SEASON_KEYWORDS}\\s*(\\d{1,2})\\b`, "i");
  const wordMatch = str.match(wordPattern);
