...
/**
 * Escapes any special regular expression metacharacters in a string.
 * @param {string} string
 * @returns {string}
 */
...
function parseStremioId(id) {
  ...
  // Strip query string fragments (e.g. ?index=0) if passed in the ID
...
function getReleaseGroup(filename) {
  ...
  // Strip query string parameters (e.g. ?index=0) before processing
  const cleanFilename = filename.split("?")[0].trim();
  const name = cleanFilename.replace(/\.[a-zA-Z0-9]+$/, "").trim();
...
function calculateMatchScore(videoFilename, subtitleFilename) {
  ...
  const hasGroupMatch =
    ...
    Boolean(vGroup && new RegExp(`\\b${escapeRegExp(vGroup)}\\b`, "i").test(subNormalized));
...
function explicitSeason(text) {
  if (!text) return null;
  const match = String(text).match(new RegExp(`(?:\\bs\\s*(\\d{1,2})\\s*e\\s*\\d{1,3}\\b|\\b(\\d{1,2})\\s*x\\s*\\d{1,3}\\b|${SEASON_KEYWORDS}\\s*(\\d{1,2})\\b)`, "i"));
  return match ? Number(match[1] || match[2] || match[3]) : null;
}

function hasExplicitEpisode(text, season, episode) {
  return matchesEpisode(text, season, episode);
}

function hasRecognizableEpisode(text) {
  if (!text) return false;
  return /\bs\s*\d{1,2}\s*e\s*\d{1,3}\b|\b\d{1,2}\s*x\s*\d{1,3}\b|\b(?:e|ep)\s*\d{1,3}\b|\b(?:episode|episod|episodio|épisode|folge|epizód|επεισόδιο|episódio)\s*\d{1,3}\b|[\-\._\s]\d{2,3}[\-\._\s\[]/i.test(String(text));
}
