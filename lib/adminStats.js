...
function renderDashboardHtml(stats, adminKey) {
  ...
  const errorRows = (stats.recentErrors || []).map((e) => `
    <tr class="border-b border-stone-100 hover:bg-stone-50/50">
      <td class="py-2.5 px-3">
        <span class="inline-block px-2 py-0.5 rounded text-[11px] font-mono font-bold bg-red-50 text-red-700 border border-red-200">${e.type}</span>
      </td>
      <td class="py-2.5 px-3 text-right font-mono font-bold text-red-600">${e.count.toLocaleString()}</td>
      <td class="py-2.5 px-3 font-mono text-slate-500 text-[11px]">${e.firstSeen ? e.firstSeen.slice(5, 19).replace('T', ' ') : '-'}</td>
      <td class="py-2.5 px-3 font-mono text-slate-500 text-[11px]">${e.lastSeen ? e.lastSeen.slice(5, 19).replace('T', ' ') : '-'}</td>
      <td class="py-2.5 px-3 font-mono text-slate-700 text-[11px]">${e.context || '-'}</td>
      <td class="py-2.5 px-3">
        <div class="text-slate-800 font-medium">${e.message}</div>
        ${e.stackSnippet ? `
          <details class="mt-1">
            <summary class="text-[11px] text-indigo-600 cursor-pointer hover:underline">View stack trace</summary>
            <pre class="mt-1 p-2 bg-slate-900 text-slate-100 rounded text-[10px] font-mono overflow-x-auto max-h-36">${e.stackSnippet}</pre>
          </details>
        ` : ''}
      </td>
    </tr>
  ...
  <body class="bg-stone-50 text-slate-900 font-sans antialiased selection:bg-indigo-100 selection:text-indigo-900">
    <main class="max-w-6xl mx-auto px-6 py-10 space-y-8">
      ...
      <header class="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-4 border-b border-stone-200 pb-6">
        <div>
          <div class="text-xs font-bold uppercase tracking-widest text-indigo-600 mb-1">Live Telemetry & Diagnostics</div>
      ...
      <!-- 4-Tier Audience & Performance Summary Cards -->
      <section class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Active (15m)</span>
          <div class="text-2xl font-mono font-bold text-emerald-600 mt-1">${stats.activeNow15m}</div>
          <span class="text-[10px] text-slate-400">live now</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Today's DAU</span>
          <div class="text-2xl font-mono font-bold text-indigo-600 mt-1">${t.uniqueActiveUsers}</div>
          <span class="text-[10px] text-slate-400">24h active</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">30-Day MAU</span>
          <div class="text-2xl font-mono font-bold text-indigo-600 mt-1">${stats.mau30d || t.uniqueActiveUsers}</div>
          <span class="text-[10px] text-slate-400">monthly active</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">All-Time</span>
          <div class="text-2xl font-mono font-bold text-slate-900 mt-1">${stats.allTimeInstalls || t.uniqueActiveUsers}</div>
          <span class="text-[10px] text-slate-400">total installs</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Requests</span>
          <div class="text-2xl font-mono font-bold text-slate-900 mt-1">${t.totalRequests.toLocaleString()}</div>
          <span class="text-[10px] text-slate-400">${t.searchRequests}s / ${t.proxyRequests}p</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Cache Hit</span>
          <div class="text-2xl font-mono font-bold ${t.cacheHitRate >= 80 ? 'text-emerald-600' : 'text-amber-600'} mt-1">${t.cacheHitRate}%</div>
          <span class="text-[10px] text-slate-400">bridge hit rate</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Avg Search</span>
          <div class="text-2xl font-mono font-bold text-slate-800 mt-1">${t.avgSearchLatencyMs}<span class="text-xs font-normal text-slate-400">ms</span></div>
          <span class="text-[10px] text-slate-400">upstream query</span>
        </div>

        <div class="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm flex flex-col justify-between">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Avg Stream</span>
          <div class="text-2xl font-mono font-bold text-slate-800 mt-1">${t.avgProxyLatencyMs}<span class="text-xs font-normal text-slate-400">ms</span></div>
          <span class="text-[10px] text-slate-400">VTT delivery</span>
        </div>
      </section>
      ...
      <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-5 rounded-xl border border-stone-200 shadow-sm space-y-3">
          ...
          <div class="space-y-2 text-xs">
            <div class="flex items-center justify-between">
              <span class="text-slate-700 font-medium">Exact Master Match (Tier 1)</span>
              <span class="font-mono font-bold text-slate-900">${t.matchTiers.exact} (${exactPct}%)</span>
            </div>
            <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
              <div class="bg-emerald-600 h-2" style="width: ${exactPct}%"></div>
            </div>

            <div class="flex items-center justify-between pt-1">
              <span class="text-slate-700 font-medium">High Scene Sync (Tiers 2-4)</span>
              <span class="font-mono font-bold text-slate-900">${t.matchTiers.highSync} (${highSyncPct}%)</span>
            </div>
            <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
              <div class="bg-indigo-600 h-2" style="width: ${highSyncPct}%"></div>
            </div>

            <div class="flex items-center justify-between pt-1">
              <span class="text-slate-700 font-medium">Medium Sync (Tiers 5-7)</span>
              <span class="font-mono font-bold text-slate-900">${t.matchTiers.medSync} (${medSyncPct}%)</span>
            </div>
            <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
              <div class="bg-blue-500 h-2" style="width: ${medSyncPct}%"></div>
            </div>

            <div class="flex items-center justify-between pt-1">
              <span class="text-slate-700 font-medium">Fuzzy Fallback (Tiers 8-10)</span>
              <span class="font-mono font-bold text-slate-900">${t.matchTiers.fallback} (${fallbackPct}%)</span>
            </div>
            <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
              <div class="bg-amber-500 h-2" style="width: ${fallbackPct}%"></div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-xl border border-stone-200 shadow-sm space-y-3">
          <div class="flex items-center justify-between border-b border-stone-100 pb-2">
            ...
            <span class="text-xs text-slate-400 font-mono">Decompression Engine</span>
          </div>
          <div class="space-y-3 text-xs pt-1">
            <div class="flex items-center justify-between">
              <span class="text-slate-700 font-medium">ZIP Archives (adm-zip)</span>
              <span class="font-mono font-bold text-slate-900">${t.archiveFormats.zip} (${zipPct}%)</span>
            </div>
            <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
              <div class="bg-emerald-600 h-2" style="width: ${zipPct}%"></div>
            </div>
      ...
      <section class="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-stone-200 flex items-center justify-between">
          <div>
            <h2 class="text-base font-bold text-slate-900">7-Day Error Diagnostics</h2>
            <p class="text-xs text-slate-400 mt-0.5">Deduplicated exception signatures & stack snippets</p>
          </div>
          <span class="text-xs font-mono text-slate-400">${(stats.recentErrors || []).length} distinct error types</span>
        </div>
          ...
          <table class="w-full text-left text-xs">
            <thead class="bg-stone-50 text-slate-500 font-semibold border-b border-stone-200 uppercase tracking-wider">
              <tr>
                ...
                <th class="py-2.5 px-3">Context</th>
                <th class="py-2.5 px-3">Message & Stack Trace</th>
              </tr>
            </thead>
            <tbody>
              ${errorRows || '<tr><td colspan="6" class="py-6 text-center text-emerald-600 font-medium">🟢 Zero errors recorded in the last 7 days. All systems operating cleanly!</td></tr>'}
            </tbody>
          </table>
        </div>
      </section>
