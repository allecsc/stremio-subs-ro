...
class SubsRoClient {
  ...
  /**
   * Stream a subtitle archive directly into a caller-owned temporary file.
   * The caller is responsible for removing the file on every outcome.
   */
  async downloadArchiveToFile(subId, destination, { beforeWrite } = {}) {
    const url = `${this.baseUrl}/subtitle/${subId}/download`;
    const response = await requestWithRetry(url, {
      headers: { "X-Subs-Api-Key": this.apiKey },
      responseType: "stream",
    ...
    try {
      for await (const value of response.data) {
        ...
        const release = beforeWrite ? await beforeWrite(chunk.length) : () => {};
        try {
