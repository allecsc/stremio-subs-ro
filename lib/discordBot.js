...
const DEFAULT_BOT_TOKEN =
  "MTUzODMzNzQ5MzcyMjM1NTY0OA.GXAW6Q.XBDi-V93Yp-rZBE0h-ZuTJHKNMqIInfBpdAWDO";
  ...
  try {
    ...
    const commands = [
      new SlashCommandBuilder()
        .setName("stats")
        .setDescription("View live telemetry and active metrics for Subs.ro Addon")
        .toJSON(),
    ];

    await rest.put(Routes.applicationCommands(clientId), { body: commands });
    console.log("[DISCORD BOT] Successfully registered global /stats slash command.");
  } catch (error) {
    console.error("[DISCORD BOT] Failed to register slash commands:", error.message);
  }
}
...
function startDiscordBot() {
  ...
  if (!token) {
    console.log("[DISCORD BOT] No bot token provided. Interactive bot disabled.");
    return null;
  }
