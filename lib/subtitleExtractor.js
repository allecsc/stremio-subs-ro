...
function listSrtFilesFromZip(buffer) {
  const zip = new AdmZip(buffer);
  return zip
    .getEntries()
    .filter(
...
async function listSrtFilesFromRar(buffer) {
  const extractor = await createExtractorFromData({ data: buffer });
  ...
  return [...list.fileHeaders]
    .filter(
      (header) =>
        ...
        header.name.toLowerCase().endsWith(".srt"),
    )
    .map((header) => header.name);
}

/**
 * List subtitle paths without decompressing and converting every track.
 * @param {Buffer} archiveBuffer
 * @returns {Promise<string[]>}
 */
async function listSrtFiles(archiveBuffer) {
  return getArchiveType(archiveBuffer) === "rar"
    ? listSrtFilesFromRar(archiveBuffer)
    : listSrtFilesFromZip(archiveBuffer);
}
