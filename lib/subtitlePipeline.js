...
function createSubtitlePipeline(options = {}) {
  ...
  const emitSchedulingEvent = (event) => {
    ...
    try {
      options.onSchedulingEvent(event);
    } catch (_) {
  ...
  const emitOperationalSignal = (signal) => {
    try {
      if (typeof options.onOperationalSignal === "function") {
        options.onOperationalSignal(Object.freeze({ ...signal }));
      } else {
      ...
      try {
        ...
        for (const entry of usableEntries) {
          ...
          try {
            ...
            console.warn(`[SUBS] Skipped unreadable ${archiveType.toUpperCase()} track "${originalPath}": ${error.message}`);
            continue;
          }
  ...
  const getArchiveSrtList = async (apiKey, subId) => {
    try {
      return await withListedPackage(apiKey, subId, undefined, async (prepared) => {
        ...
        console.log(`[${ts}] [SUBS] Archive ${subId}: ${prepared.tracks.length} usable SRTs (ZIP) -> Prepared on disk`);
    ...
    try {
      while (true) {
        ...
        try {
          ...
          const track = prepared.tracks.find((candidate) => candidate.id === encodedSrtPath);
